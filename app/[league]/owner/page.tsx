...
function CapBar({ contracts, retentions }: { contracts: Contract[]; retentions: CapRetention[] }) {
  ...
  const dedupedContracts = contracts.filter((c, i, arr) => arr.findIndex((x) => x.mc_uuid === c.mc_uuid) === i);
...
function SigningsTab({ teamId, leagueSlug, contracts, onRefresh }: {
  ...
  const makeOffer = async () => {
    ...
    setSubmitting(false);
    if (!r.ok) return setErr(d.error);
    setSuccessMsg(`Offer sent to ${selectedInfo?.mc_username ?? "player"}${!hideCap ? ` for $${fmt(amt)}` : ""}. They've been DM'd and can accept right away via Discord or their profile.`);
